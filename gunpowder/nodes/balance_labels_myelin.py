...
class BalanceLabelsMyelin(BatchFilter):
    ...
    def process(self, batch, request):
        ...
        labels_raw = batch.arrays[self.labels]

        np.place(labels, labels == 255, 1)  # raw myelin is labeled either 0 or 255
        ...
        for key in self.masks:
            ...
            if mask.data.shape == (
                    ...
                    labels.data.shape[1] - 1,
                    labels.data.shape[2] - 1,
                    ):
                # if mask is 1 voxel off, correct it
                mask.data = mask.data[1:, 1:, 1:]
